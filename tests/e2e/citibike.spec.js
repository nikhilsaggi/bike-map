...
test.describe('Citibike dock layer', () => {
  ...
  test('clicking a dock draws its own trips and nothing else', async ({ page }) => {
    ...
    await expect(panel).toContainText('Home Dock & Main St');
    await expect(panel).toContainText('4 trips');
    await expect(panel).toContainText('3 out / 1 in');
    await expect(panel).toContainText('2 docks reached');
    // Two lines, not one: on a busy dock this line was the widest thing in
    // the box and set the width for every row under it.
    await expect(panel.locator('.dock-sub > div')).toHaveCount(2);
    await expect(panel.locator('.ride-row').first()).toContainText('Park Dock & 5 Ave');

    // Closing the panel takes the lines with it.
  ...
  test('a dock row jumps to that dock, so the network is walkable', async ({ page }) => {
    await gotoMap(page);
    await showDocks(page);
    await page.evaluate(() => selectDock(0));
    const panel = page.locator('#inspector');
    await expect(page.locator('#inspector-title')).toHaveText('Home Dock & Main St');
    ...
    await panel.locator('.ride-row', { hasText: 'Terminal Dock' }).locator('.dock-name').click();
    await expect(page.locator('#inspector-title'))
  ...
  test('a row reads name first, counts and route under it', async ({ page }) => {
    ...
    await showDocks(page);
    await page.evaluate(() => selectDock(0));
    const park = page.locator('#inspector .ride-row', { hasText: 'Park Dock' });
    ...
    // the box as wide as the stats panel across the map.
  ...
  test('a row offers the recording of the trip, and only where there is one', async ({ page }) => {
    ...
    const park = panel.locator('.ride-row', { hasText: 'Park Dock' });
    ...
    const terminal = panel.locator('.ride-row', { hasText: 'Terminal Dock' });
    await expect(terminal.locator('.dock-trace')).toHaveCount(0);
  });

  test('showing a recording keeps the panel, the line and the ride view', async ({ page }) => {
    await gotoMap(page);
    await showDocks(page);
    await page.evaluate(() => selectDock(0));
    const panel = page.locator('#inspector');
    await panel.locator('.ride-row', { hasText: 'Park Dock' }).locator('.dock-trace').click();
    ...
    // The panel is how a reader walks the network, and it covers no map, so
    // tracing a pair leaves it exactly where it was.
    await expect(page.locator('#inspector-title')).toHaveText('Home Dock & Main St');
  ...
  test('says when the recording holds more than the trip that was clicked', async ({ page }) => {
    ...
    await page.evaluate(() => selectDock(0));
    const panel = page.locator('#inspector');
    const trace = panel.locator('.ride-row', { hasText: 'Park Dock' }).locator('.dock-trace');
    ...
    // this hop.
    await trace.click();
    await expect(panel.locator('.dock-trace-note'))
    ...
    await expect(panel.locator('.dock-trace-note')).toBeHidden();
  });
  ...
  test('clicking past the last recording puts the map back', async ({ page }) => {
    ...
    await panel.locator('.ride-row', { hasText: 'Park Dock' }).locator('.dock-name').click();
    await expect(page.locator('#inspector-title'))
      .toHaveText('Park Dock & 5 Ave');
  });

  test('escape unwinds the route first, then the dock behind it', async ({ page }) => {
    await gotoMap(page);
    await showDocks(page);
    await page.evaluate(() => selectDock(0));
    const trace = page.locator('#inspector .ride-row', { hasText: 'Park Dock' })
      .locator('.dock-trace');
    await trace.click();
    await expect(trace).toHaveText('\u25b8 route 1/2');

    // One key, one layer: the route comes off and the dock it came from is
    // still open with every link back, because backing out of one detour is
    // not the same as being finished with the dock.
    ...
    // A second one puts the panel away, and the dock with it.
    await page.keyboard.press('Escape');
    ...
    // back, because the rows are rebuilt from scratch each time.
    await page.evaluate(() => selectDock(0));
    await expect(page.locator('#inspector .ride-row', { hasText: 'Park Dock' })
  ...
  test('the arrows belong to the cycle, not to the map under it', async ({ page }) => {
    ...
    await page.evaluate(() => selectDock(0));
    await page.locator('#inspector .ride-row', { hasText: 'Park Dock' })
  ...
  test('a pair with one recording gets no position in the bar', async ({ page }) => {
    ...
    await page.evaluate(() => selectDock(2));
    await page.locator('#inspector .dock-row-flat .dock-trace').click();
    await expect(page.locator('#ride-view-bar')).toBeVisible();
  ...
  test('a ride shown from anywhere else takes no arrow keys with it', async ({ page }) => {
    ...
    // viewRide from a street's rows or a year link is not a dock pair's cycle,
  ...
  test('a recording is offered even where the far dock cannot be placed', async ({ page }) => {
    ...
    await page.evaluate(() => selectDock(2));
    const ghost = page.locator('#inspector .dock-row-flat');
  ...
  test('a payload from before the trips carried a ride offers no routes', async ({ page }) => {
    ...
    await showDocks(page);
    await page.evaluate(() => selectDock(0));
    await expect(page.locator('#inspector .dock-trace')).toHaveCount(0);
    // The rest of the layer is untouched by the missing element.
    await expect(page.locator('#inspector .ride-row')).toHaveCount(2);
  });
  ...
  test('a dock with no coordinates is listed but not offered as a link', async ({ page }) => {
    ...
    await page.evaluate(() => selectDock(2));
    const panel = page.locator('#inspector');
    await expect(panel.locator('.dock-row-flat')).toHaveCount(1);
    await expect(panel.locator('.dock-row-flat')).toContainText('Ghost Dock & Gone St');
    await expect(panel.locator('.ride-row')).toHaveCount(1);
    await expect(panel.locator('.ride-row')).toContainText('Home Dock & Main St');
  });
  ...
  test('closing the dock while its route is up leaves ride view alone', async ({ page }) => {
    ...
    await page.evaluate(() => closeInspector());
    // The route is still the subject, so the panel closing must not repaint
    // the heatmap over it.
    expect(await page.evaluate(() => rideView)).toBe(3);
    expect((await edgeColors(page)).filter(c => c === '#00e5ff')).toHaveLength(2);
  });
...
test.describe('ride source', () => {
  ...
  test('labels a ride wherever it is already named', async ({ page }) => {
    ...
    const rows = panel.locator('.ride-row');
  ...
  test('the panel lists only the rides the filter kept', async ({ page }) => {
    await gotoMap(page);
    await page.click('.seg-btn[data-src="own"]');
    await clickEdge(page, EDGES.center.lat);
    const panel = page.locator('#inspector');
    await expect(panel.locator('.ride-row')).toHaveCount(1);
    await expect(panel.locator('.src-tag.own')).toHaveText('own bike');
  });
