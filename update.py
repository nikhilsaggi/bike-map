...
Arguments are passed through to garmin_sync.py. The commit is left unpushed
on purpose -- check the map, then `git push`.
...
def main() -> int:
    ...
    py = sys.executable
    try:
        _run(py, "garmin_sync.py", "incoming", *sys.argv[1:])
        _run(py, "gpx_to_csv.py", "incoming", "rides")
        ...
        # on top of that just buries it.
        print(f"\nUpdate stopped: {exc.args[1][1]} exited {exc.returncode}.", file=sys.stderr)
